...
export class TarefaListComponent implements OnInit, OnDestroy /*{CA:CLASS_INTERFACES:START}*/ /*{CA:CLASS_INTERFACES:END}*/ {
    public allPermissions: any = {};
    public permissions: any = {};
    public localeConfig: any = {};

    public currentListParams: ListParams = { page: 0, size: 10, sort: [], filterData: {} };
    public gridData: Tarefa[];
    public gridColumns: any[];
    public showLoader: boolean;
    public totalRecords: number;
    public selection: Tarefa[];
    ...
    @ViewChild("tarefaTable", { static: false })
    public table: Table;

    public participantesLookupDisplayFields = ["id", "nome", "idade"];
    public participantesAdvancedDisplayFields = ["id", "nome", "idade"];
    ...
    public onSearch() {
        ...
        if (this.hasEmpty(filterData)) {
            ...
            this.filtersStorageService.storeFilters("TarefaListComponent", filterData);
        }
    ...
    public onClear() {
        ...
        const filterData = this.filterFormGroup.getRawValue();
        this.resetGrid({ filterData });
        this.filtersStorageService.removeFilter("TarefaListComponent");
    ...
    public onRemoveToken(token: IToken) {
        ...
        const filterData = this.filterFormGroup.getRawValue();
        this.resetGrid({ filterData });
        this.filtersStorageService.clearFilterValue("TarefaListComponent", filterData);
    ...
    public onGridChange(event: LazyLoadEvent) {
        ...
        if (size != 10 || page != 0 || sort.length > 0) {
            this.paginatorStorageService.storePaginator("TarefaListComponentPaginator", { page, size, sort });
        }
    ...
    public onDelete() {
        ...
        this.confirmationService.confirm({
            message: this.translate.instant("godev.godev2022.delete_confirmation_message"),
            header: this.translate.instant("godev.godev2022.delete_confirmation_title"),
            accept: () => {
                forkJoin(this.selection.map(tarefa => this.tarefaService.delete(tarefa.id)))
                    .pipe(
                    ...
                    .subscribe(
                        () => {
                            ...
                            this.messageService.add({
                                severity: "success",
                                summary: this.translate.instant("godev.godev2022.deleted_message_title"),
                                detail: this.translate.instant("godev.godev2022.deleted_message_content"),
                            });
    ...
    private getGridColumns() {
        const gridColumns = [
            { field: "id", header: this.translate.instant("godev.godev2022.tarefa_id") },
            { field: "descricao", header: this.translate.instant("godev.godev2022.tarefa_descricao") },
        ];
    ...
    private getFilterFields() {
        const filterFields = [
            new FormField({
                name: "id",
                label: this.translate.instant("godev.godev2022.tarefa_id"),
                type: FieldType.String,
            }),
            new FormField({
                name: "descricao",
                label: this.translate.instant("godev.godev2022.tarefa_descricao"),
                type: FieldType.String,
            }),
        ];
    ...
    private async setStorageFiltersIntoForm(form: FormGroup) {
        ...
        const filters = await this.filtersStorageService.getFilters("TarefaListComponent");
    ...
    private async setStoragePaginator() {
        const paginator = await this.paginatorStorageService.getPaginator("TarefaListComponentPaginator");
