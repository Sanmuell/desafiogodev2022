...
public class ResponsibleFamilyMemberImporterListener implements ImporterListener {
	
	private static final Logger logger = LoggerFactory.getLogger(ResponsibleFamilyMemberImporterListener.class);
	...
	private void publishEventStatus(ImportEventStatus status) {
		...
		if (runner != null) {
			runner.publish(ServiceContext.get().getCurrentTenant(),  Godev2022desafioConstants.Events.IMPORT_RESPONSIBLE_FAMILY_MEMBER_EVENT, new ImportResponsibleFamilyMemberEventPayload(status));
		}
	}
