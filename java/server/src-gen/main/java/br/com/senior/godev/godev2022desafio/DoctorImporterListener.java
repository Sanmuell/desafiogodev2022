...
public class DoctorImporterListener implements ImporterListener {
	
	private static final Logger logger = LoggerFactory.getLogger(DoctorImporterListener.class);
	...
	private void publishEventStatus(ImportEventStatus status) {
		...
		if (runner != null) {
			runner.publish(ServiceContext.get().getCurrentTenant(),  Godev2022desafioConstants.Events.IMPORT_DOCTOR_EVENT, new ImportDoctorEventPayload(status));
		}
	}
