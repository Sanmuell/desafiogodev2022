...
public class PatientImporterListener implements ImporterListener {
	
	private static final Logger logger = LoggerFactory.getLogger(PatientImporterListener.class);
	...
	private void publishEventStatus(ImportEventStatus status) {
		...
		if (runner != null) {
			runner.publish(ServiceContext.get().getCurrentTenant(),  Godev2022desafioConstants.Events.IMPORT_PATIENT_EVENT, new ImportPatientEventPayload(status));
		}
	}
