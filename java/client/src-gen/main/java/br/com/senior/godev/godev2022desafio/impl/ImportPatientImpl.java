...
import br.com.senior.godev.godev2022desafio.ImportPatientInput;
import br.com.senior.godev.godev2022desafio.ImportPatientOutput;
 ...
 * 
 */
public class ImportPatientImpl {
	...
	public ImportPatientImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportPatientInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				Godev2022desafioConstants.DOMAIN, //
				Godev2022desafioConstants.SERVICE, //
				Godev2022desafioConstants.Commands.IMPORT_PATIENT, //
	...
	*/
	public ImportPatientOutput importPatient(ImportPatientInput input, long timeout) {
		...
		ImportPatientOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportPatientOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ImportPatientOutput> importPatientRequest(ImportPatientInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportPatientOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ImportPatientOutput> importPatientRequest(ImportPatientInput input, long timeout, TimeUnit unit) {
		Godev2022desafioValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportPatientOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
