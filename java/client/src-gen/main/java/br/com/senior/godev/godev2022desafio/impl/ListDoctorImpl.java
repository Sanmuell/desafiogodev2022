...
public class ListDoctorImpl {
	...
	public ListDoctorImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Doctor.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				Godev2022desafioConstants.DOMAIN, //
				Godev2022desafioConstants.SERVICE, //
				Godev2022desafioConstants.Commands.LIST_DOCTOR, //
	...
	*/
	public Doctor.PagedResults listDoctor(Doctor.PageRequest input, long timeout) {
		...
		Doctor.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Doctor.PagedResults.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<Doctor.PagedResults> listDoctorRequest(Doctor.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Doctor.PagedResults.class);
	}
	...
	* The 'list' request primitive for the Doctor entity.
	*/
	public CompletableFuture<Doctor.PagedResults> listDoctorRequest(Doctor.PageRequest input, long timeout, TimeUnit unit) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Doctor.PagedResults.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
