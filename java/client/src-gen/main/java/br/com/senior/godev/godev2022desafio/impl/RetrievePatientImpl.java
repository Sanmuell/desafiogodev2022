 ...
 * 
 */
public class RetrievePatientImpl {
	...
	public RetrievePatientImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Patient.GetRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				Godev2022desafioConstants.DOMAIN, //
				Godev2022desafioConstants.SERVICE, //
				Godev2022desafioConstants.Commands.RETRIEVE_PATIENT, //
	...
	*/
	public Patient retrievePatient(Patient.GetRequest input, long timeout) {
		...
		Patient output = DtoJsonConverter.toDTO(resultMessage.getContent(), Patient.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<Patient> retrievePatientRequest(Patient.GetRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Patient.class);
	}
	...
	* The 'retrieve' request primitive for the Patient entity.
	*/
	public CompletableFuture<Patient> retrievePatientRequest(Patient.GetRequest input, long timeout, TimeUnit unit) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Patient.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
