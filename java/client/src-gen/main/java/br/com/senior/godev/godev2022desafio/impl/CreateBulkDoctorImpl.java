...
import br.com.senior.godev.godev2022desafio.CreateBulkDoctorInput;
import br.com.senior.godev.godev2022desafio.CreateBulkDoctorOutput;
 ...
 * 
 */
public class CreateBulkDoctorImpl {
	...
	public CreateBulkDoctorImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(CreateBulkDoctorInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				Godev2022desafioConstants.DOMAIN, //
				Godev2022desafioConstants.SERVICE, //
				Godev2022desafioConstants.Commands.CREATE_BULK_DOCTOR, //
	...
	*/
	public CreateBulkDoctorOutput createBulkDoctor(CreateBulkDoctorInput input, long timeout) {
		...
		CreateBulkDoctorOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), CreateBulkDoctorOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<CreateBulkDoctorOutput> createBulkDoctorRequest(CreateBulkDoctorInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, CreateBulkDoctorOutput.class);
	}
	...
	* The 'createBulk' request primitive for the Doctor entity.
	*/
	public CompletableFuture<CreateBulkDoctorOutput> createBulkDoctorRequest(CreateBulkDoctorInput input, long timeout, TimeUnit unit) {
		Godev2022desafioValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, CreateBulkDoctorOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
