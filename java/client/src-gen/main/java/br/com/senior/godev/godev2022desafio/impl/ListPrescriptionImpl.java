 ...
 * 
 */
public class ListPrescriptionImpl {
	...
	public ListPrescriptionImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Prescription.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				Godev2022desafioConstants.DOMAIN, //
				Godev2022desafioConstants.SERVICE, //
				Godev2022desafioConstants.Commands.LIST_PRESCRIPTION, //
	...
	*/
	public Prescription.PagedResults listPrescription(Prescription.PageRequest input, long timeout) {
		...
		Prescription.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Prescription.PagedResults.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<Prescription.PagedResults> listPrescriptionRequest(Prescription.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Prescription.PagedResults.class);
	}
	...
	* The 'list' request primitive for the Prescription entity.
	*/
	public CompletableFuture<Prescription.PagedResults> listPrescriptionRequest(Prescription.PageRequest input, long timeout, TimeUnit unit) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Prescription.PagedResults.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
