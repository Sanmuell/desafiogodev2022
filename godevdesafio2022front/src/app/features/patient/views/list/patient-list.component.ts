...
export class PatientListComponent implements OnInit, OnDestroy /*{CA:CLASS_INTERFACES:START}*/ /*{CA:CLASS_INTERFACES:END}*/ {
    ...
    @ViewChild("patientTable", { static: false })
    public table: Table;

    public responsibleDoctorsLookupDisplayFields = ["id", "name", "cpf", "crmNumber", "crmState", "especiality"];
    public responsibleDoctorsAdvancedDisplayFields = ["id", "name", "cpf", "crmNumber", "crmState", "especiality"];

    public prescriptionsLookupDisplayFields = ["id", "drug", "description", "data", "type", "statementOfReponsability"];
    public prescriptionsAdvancedDisplayFields = ["id", "drug", "description", "data", "type", "statementOfReponsability"];

    public responsibleFamilyMembersLookupDisplayFields = ["id", "name", "cpf", "kinship"];
    public responsibleFamilyMembersAdvancedDisplayFields = ["id", "name", "cpf", "kinship"];
    ...
    public ngOnInit() {
        ...
        this.filterFormGroup = this.formBuilder.group({
            ...
            cpf: [undefined, Validators.compose([])],
            address: [undefined, Validators.compose([])],
            phone: [undefined, Validators.compose([Validators.pattern(/^\-?\d*$/)])],
            isAtivo: [undefined, Validators.compose([])],
        });
    ...
    public onSearch() {
        ...
        if (this.hasEmpty(filterData)) {
            ...
            this.filtersStorageService.storeFilters("PatientListComponent", filterData);
        }
    ...
    public onClear() {
        ...
        const filterData = this.filterFormGroup.getRawValue();
        this.resetGrid({ filterData });
        this.filtersStorageService.removeFilter("PatientListComponent");
    ...
    public onRemoveToken(token: IToken) {
        ...
        const filterData = this.filterFormGroup.getRawValue();
        this.resetGrid({ filterData });
        this.filtersStorageService.clearFilterValue("PatientListComponent", filterData);
    ...
    public onGridChange(event: LazyLoadEvent) {
        ...
        if (size != 10 || page != 0 || sort.length > 0) {
            this.paginatorStorageService.storePaginator("PatientListComponentPaginator", { page, size, sort });
        }
    ...
    public onDelete() {
        ...
        this.confirmationService.confirm({
            ...
            accept: () => {
                forkJoin(this.selection.map(patient => this.patientService.delete(patient.id)))
                    .pipe(
                    ...
                    .subscribe(
                        () => {
                            ...
                            this.messageService.add({
                                severity: "success",
                                summary: this.translate.instant("godev.godev2022desafio.deleted_message_title"),
                                detail: this.translate.instant("godev.godev2022desafio.deleted_message_content"),
                            });
    ...
    private getGridColumns() {
        const gridColumns = [
            { field: "id", header: this.translate.instant("godev.godev2022desafio.patient_id") },
            { field: "name", header: this.translate.instant("godev.godev2022desafio.patient_name") },
            { field: "cpf", header: this.translate.instant("godev.godev2022desafio.patient_cpf") },
            { field: "address", header: this.translate.instant("godev.godev2022desafio.patient_address") },
            { field: "phone", header: this.translate.instant("godev.godev2022desafio.patient_phone") },
            { field: "isAtivo", header: this.translate.instant("godev.godev2022desafio.patient_is_ativo") },
        ];
    ...
    private async setStorageFiltersIntoForm(form: FormGroup) {
        ...
        const filters = await this.filtersStorageService.getFilters("PatientListComponent");
    ...
    private async setStoragePaginator() {
        const paginator = await this.paginatorStorageService.getPaginator("PatientListComponentPaginator");
