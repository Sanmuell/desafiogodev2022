...
@Component({
    templateUrl: "./responsible-family-member-list.component.html",
    styleUrls: [
...
export class ResponsibleFamilyMemberListComponent
    implements OnInit, OnDestroy /*{CA:CLASS_INTERFACES:START}*/ /*{CA:CLASS_INTERFACES:END}*/ {
    public allPermissions: any = {};
    public permissions: any = {};
    public localeConfig: any = {};

    public currentListParams: ListParams = { page: 0, size: 10, sort: [], filterData: {} };
    public gridData: ResponsibleFamilyMember[];
    public gridColumns: any[];
    public showLoader: boolean;
    public totalRecords: number;
    public selection: ResponsibleFamilyMember[];
    ...
    @ViewChild("responsibleFamilyMemberTable", { static: false })
    public table: Table;
    ...
    public ngOnInit() {
        ...
        this.filterFormGroup = this.formBuilder.group({
            ...
            kinship: [undefined, Validators.compose([])],
        });
    ...
    public onRouteDataChange(data: any) {
        ...
        this.permissions = data.allPermissions.responsibleFamilyMember;
    ...
    public onSearch() {
        ...
        if (this.hasEmpty(filterData)) {
            ...
            this.filtersStorageService.storeFilters("ResponsibleFamilyMemberListComponent", filterData);
        }
    ...
    public onClear() {
        ...
        const filterData = this.filterFormGroup.getRawValue();
        this.resetGrid({ filterData });
        this.filtersStorageService.removeFilter("ResponsibleFamilyMemberListComponent");
    ...
    public onRemoveToken(token: IToken) {
        ...
        const filterData = this.filterFormGroup.getRawValue();
        this.resetGrid({ filterData });
        this.filtersStorageService.clearFilterValue("ResponsibleFamilyMemberListComponent", filterData);
    ...
    public onGridChange(event: LazyLoadEvent) {
        ...
        if (size != 10 || page != 0 || sort.length > 0) {
            this.paginatorStorageService.storePaginator("ResponsibleFamilyMemberListComponentPaginator", { page, size, sort });
        }
    ...
    public onDelete() {
        ...
        this.confirmationService.confirm({
            ...
            accept: () => {
                forkJoin(
                    this.selection.map(responsibleFamilyMember => this.responsibleFamilyMemberService.delete(responsibleFamilyMember.id))
                )
                    .pipe(
    ...
    private getGridColumns() {
        const gridColumns = [
            { field: "id", header: this.translate.instant("godev.godev2022desafio.responsible_family_member_id") },
            { field: "name", header: this.translate.instant("godev.godev2022desafio.responsible_family_member_name") },
            { field: "cpf", header: this.translate.instant("godev.godev2022desafio.responsible_family_member_cpf") },
            { field: "kinship", header: this.translate.instant("godev.godev2022desafio.responsible_family_member_kinship") },
        ];
    ...
    private async setStorageFiltersIntoForm(form: FormGroup) {
        ...
        const filters = await this.filtersStorageService.getFilters("ResponsibleFamilyMemberListComponent");
    ...
    private async setStoragePaginator() {
        const paginator = await this.paginatorStorageService.getPaginator("ResponsibleFamilyMemberListComponentPaginator");
